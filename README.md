...
- functions - Code for the application's Lambda functions to handle the Cloudability integration workflow.
- statemachines - Definition for the state machine that orchestrates the Cloudability integration workflow.
- template.yaml - A template that defines the application's AWS resources.

This application creates a Cloudability integration workflow which runs on a pre-defined schedule (note that the schedule is disabled by default to avoid incurring charges). It demonstrates the power of Step Functions to orchestrate Lambda functions and other AWS resources to form complex and robust workflows, coupled with event-driven development using Amazon EventBridge.
...
```bash
cldy$ sam logs -n CldyListAccountsFunction --stack-name cldy --tail
```
...
```bash
aws cloudformation delete-stack --stack-name cldy
```
